...
local function p()
	...
	local q = b()
	local OldIndex
	OldIndex = hookmetamethod(game, "__index", newcclosure(function(self, v)
		...
		if d[self] and d[self][v] then
			...
			if w.IsCallback then
				return w.Value(self, OldIndex(self, v)) or OldIndex(self, v)
			else
				return w.Value or OldIndex(self, v)
			end
			...
			for y, z in next, i do
				if z[v] then
					...
					if A.IsCallback then
						return A.Value(self) or OldIndex(self, v)
					else
						return A.Value or OldIndex(self, v)
					end
					break
				end
			end
	...
	local OldNewIndex
	OldNewIndex = hookmetamethod(game, "__newindex", newcclosure(function(self, v, B)
		if not getgenv().MTAPIDebug and q() then
			return OldNewIndex(self, v, B)
		end
		if f[self] and f[self][v] then
			local C = f[self][v]
			if C.IsCallback then
				local D = C.Value(self, B)
				return OldNewIndex(self, v, D or B)
			else
				return OldNewIndex(self, v, C.Value or B)
			end
			...
			for y, z in next, j do
				if z[v] then
					...
					if E.IsCallback then
						...
						return OldNewIndex(self, v, E.Value or B)
					end
					break
				end
			end
		...
		if not checkcaller() or getgenv().MTAPIDebug then
			...
			for y, z in next, k do
				if z[G] then
					local T = z[G]
					if T.Callback then
						return T.Callback(self, OldNamecall, ...) or { Failure = true }
					end
					error("mt-api: Callback is nil")
					break
				end
